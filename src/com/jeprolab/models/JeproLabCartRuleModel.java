 ...
 */
public class JeproLabCartRuleModel extends JeproLabModel {
   ...
   /* public $date_from;
    ...
    public function delete()
    {
        ...
        $r &= Db::getInstance()->delete('cart_rule_product_rule', 'NOT EXISTS (SELECT 1 FROM `'.dataBaseObject.quoteName(".'cart_rule_product_rule_group`
        ...
        $r &= Db::getInstance()->delete('cart_rule_product_rule_value', 'NOT EXISTS (SELECT 1 FROM `'.dataBaseObject.quoteName(".'cart_rule_product_rule`
        WHERE `'.dataBaseObject.quoteName(".'cart_rule_product_rule_value`.`id_product_rule` = `'.dataBaseObject.quoteName(".'cart_rule_product_rule`.`id_product_rule`)');
    ...
    public static function copyConditions($id_cart_rule_source, $id_cart_rule_destination)
    {
        Db::getInstance()->execute('
            INSERT INTO `'.dataBaseObject.quoteName(".'cart_rule_shop` (`id_cart_rule`, `id_shop`)
        (SELECT '.(int)$id_cart_rule_destination.', id_shop FROM `'.dataBaseObject.quoteName(".'cart_rule_shop` WHERE `id_cart_rule` = '.(int)$id_cart_rule_source.')');
        Db::getInstance()->execute('
            INSERT INTO `'.dataBaseObject.quoteName(".'cart_rule_carrier` (`id_cart_rule`, `id_carrier`)
        (SELECT '.(int)$id_cart_rule_destination.', id_carrier FROM `'.dataBaseObject.quoteName(".'cart_rule_carrier` WHERE `id_cart_rule` = '.(int)$id_cart_rule_source.')');
        Db::getInstance()->execute('
            INSERT INTO `'.dataBaseObject.quoteName(".'cart_rule_group` (`id_cart_rule`, `id_group`)
        (SELECT '.(int)$id_cart_rule_destination.', id_group FROM `'.dataBaseObject.quoteName(".'cart_rule_group` WHERE `id_cart_rule` = '.(int)$id_cart_rule_source.')');
        Db::getInstance()->execute('
            INSERT INTO `'.dataBaseObject.quoteName(".'cart_rule_country` (`id_cart_rule`, `id_country`)
        (SELECT '.(int)$id_cart_rule_destination.', id_country FROM `'.dataBaseObject.quoteName(".'cart_rule_country` WHERE `id_cart_rule` = '.(int)$id_cart_rule_source.')');
        Db::getInstance()->execute('
            INSERT INTO `'.dataBaseObject.quoteName(".'cart_rule_combination` (`id_cart_rule_1`, `id_cart_rule_2`)
        (SELECT '.(int)$id_cart_rule_destination.', IF(id_cart_rule_1 != '.(int)$id_cart_rule_source.', id_cart_rule_1, id_cart_rule_2) FROM `'.dataBaseObject.quoteName(".'cart_rule_combination`
        ...
        // Db::getInstance()->execute('DELETE FROM `'.dataBaseObject.quoteName(".'cart_rule_product_rule` WHERE `id_cart_rule` = '.(int)$this->id);
        // Db::getInstance()->execute('DELETE FROM `'.dataBaseObject.quoteName(".'cart_rule_product_rule_value` WHERE `id_product_rule` NOT IN (SELECT `id_product_rule` FROM `'.dataBaseObject.quoteName(".'cart_rule_product_rule`)');

        // Copy products/category filters
        $products_rules_group_source = Db::getInstance()->ExecuteS('
            SELECT id_product_rule_group,quantity FROM `'.dataBaseObject.quoteName(".'cart_rule_product_rule_group`
        WHERE `id_cart_rule` = '.(int)$id_cart_rule_source.' ');

        foreach ($products_rules_group_source as $product_rule_group_source) {
        Db::getInstance()->execute('
                INSERT INTO `'.dataBaseObject.quoteName(".'cart_rule_product_rule_group` (`id_cart_rule`, `quantity`)
        ...
        $products_rules_source = Db::getInstance()->ExecuteS('
                SELECT id_product_rule,type FROM `'.dataBaseObject.quoteName(".'cart_rule_product_rule`
        ...
        foreach ($products_rules_source as $product_rule_source) {
            Db::getInstance()->execute('
                    INSERT INTO `'.dataBaseObject.quoteName(".'cart_rule_product_rule` (`id_product_rule_group`, `type`)
            ...
            $products_rules_values_source = Db::getInstance()->ExecuteS('
                    SELECT id_item FROM `'.dataBaseObject.quoteName(".'cart_rule_product_rule_value`
            ...
            foreach ($products_rules_values_source as $product_rule_value_source) {
                Db::getInstance()->execute('
                        INSERT INTO `'.dataBaseObject.quoteName(".'cart_rule_product_rule_value` (`id_product_rule`, `id_item`)
    ...
    public static function getIdByCode($code)
    {
        ...
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('SELECT `id_cart_rule` FROM `'.dataBaseObject.quoteName(".'cart_rule` WHERE `code` = \''.pSQL($code).'\'');
    }
    ...
    public static function getCustomerCartRules($id_lang, $id_customer, $active = false, $includeGeneric = true, $inStock = false, Cart $cart = null, $free_shipping_only = false, $highlight_only = false)
    {
        ...
        $sql_part1 = '* FROM `'.dataBaseObject.quoteName(".'cart_rule` cr
        LEFT JOIN `'.dataBaseObject.quoteName(".'cart_rule_lang` crl ON (cr.`id_cart_rule` = crl.`id_cart_rule` AND crl.`id_lang` = '.(int)$id_lang.')';
        ...
        if ($cart_rule['group_restriction']) {
            $cartRuleGroups = Db::getInstance()->executeS('SELECT id_group FROM '.dataBaseObject.quoteName(".'cart_rule_group WHERE id_cart_rule = '.(int)$cart_rule['id_cart_rule']);
        ...
        if ($cart_rule['shop_restriction']) {
            $cartRuleShops = Db::getInstance()->executeS('SELECT id_shop FROM '.dataBaseObject.quoteName(".'cart_rule_shop WHERE id_cart_rule = '.(int)$cart_rule['id_cart_rule']);
        ...
        if ($cart_rule['country_restriction']) {
            ...
            $countries = Db::getInstance()->ExecuteS('
                    SELECT `id_country`
                    FROM `'.dataBaseObject.quoteName(".'address`
            ...
            if (is_array($countries) && count($countries)) {
                foreach ($countries as $country) {
                    $id_cart_rule = (bool)Db::getInstance()->getValue('
                            ...
                            FROM '.dataBaseObject.quoteName(".'cart_rule_country crc
    ...
    public function usedByCustomer($id_customer)
    {
        return (bool)Db::getInstance()->getValue('
            SELECT id_cart_rule
            FROM `'.dataBaseObject.quoteName(".'order_cart_rule` ocr
        LEFT JOIN `'.dataBaseObject.quoteName(".'orders` o ON ocr.`id_order` = o.`id_order`
    ...
    public static function cartRuleExists($name)
    {
        ...
        return (bool)Db::getInstance()->getValue('
            SELECT `id_cart_rule`
            FROM `'.dataBaseObject.quoteName(".'cart_rule`
    ...
    public function getProductRuleGroups()
    {
        ...
        $result = Db::getInstance()->executeS('SELECT * FROM '.dataBaseObject.quoteName(".'cart_rule_product_rule_group WHERE id_cart_rule = '.(int)$this->id);
    ...
    public function getProductRules($id_product_rule_group)
    {
        ...
        $results = Db::getInstance()->executeS('
            SELECT *
                    FROM '.dataBaseObject.quoteName(".'cart_rule_product_rule pr
        LEFT JOIN '.dataBaseObject.quoteName(".'cart_rule_product_rule_value prv ON pr.id_product_rule = prv.id_product_rule
    ...
    public function checkValidity(Context $context, $alreadyInCart = false, $display_error = true, $check_carrier = true)
    {
        ...
        if ($context->cart->id_customer) {
            $quantityUsed = Db::getInstance()->getValue('
                    SELECT count(*)
            FROM '.dataBaseObject.quoteName(".'orders o
            LEFT JOIN '.dataBaseObject.quoteName(".'order_cart_rule od ON o.id_order = od.id_order
        ...
        if ($this->group_restriction) {
            $id_cart_rule = (int)Db::getInstance()->getValue('
                    ...
                    FROM '.dataBaseObject.quoteName(".'cart_rule_group crg
            WHERE crg.id_cart_rule = '.(int)$this->id.'
            AND crg.id_group '.($context->cart->id_customer ? 'IN (SELECT cg.id_group FROM '.dataBaseObject.quoteName(".'customer_group cg WHERE cg.id_customer = '.(int)$context->cart->id_customer.')' : '= '.(int)Configuration::get('PS_UNIDENTIFIED_GROUP')));
            if (!$id_cart_rule) {
        ...
        if ($this->country_restriction) {
            ...
            AND crc.id_country = (SELECT a.id_country FROM '.dataBaseObject.quoteName(".'address a WHERE a.id_address = '.(int)$context->cart->id_address_delivery.' LIMIT 1)');
            if (!$id_cart_rule) {
        ...
        if ($this->carrier_restriction && $check_carrier) {
            ...
            $id_cart_rule = (int)Db::getInstance()->getValue('
                    SELECT crc.id_cart_rule
                    FROM '.dataBaseObject.quoteName(".'cart_rule_carrier crc
            INNER JOIN '.dataBaseObject.quoteName(".'carrier c ON (c.id_reference = crc.id_carrier AND c.deleted = 0)
            WHERE crc.id_cart_rule = '.(int)$this->id.'
        ...
        $id_cart_rule = (int)Db::getInstance()->getValue('
                ...
                FROM '.dataBaseObject.quoteName(".'cart_rule_shop crs
            ...
            foreach ($otherCartRules as $otherCartRule) {
                ...
                if ($this->cart_rule_restriction && $otherCartRule['cart_rule_restriction'] && $otherCartRule['id_cart_rule'] != $this->id) {
                    $combinable = Db::getInstance()->getValue('
                            ...
                            FROM '.dataBaseObject.quoteName(".'cart_rule_combination
                    ...
                    switch ($product_rule['type']) {
                        case 'attributes':
                            $cart_attributes = Db::getInstance()->executeS('
                                ...
                                FROM `'.dataBaseObject.quoteName(".'cart_product` cp
                            LEFT JOIN `'.dataBaseObject.quoteName(".'product_attribute_combination` pac ON cp.id_product_attribute = pac.id_product_attribute
                        ...
                        case 'categories':
                            $cart_categories = Db::getInstance()->executeS('
                                ...
                                FROM `'.dataBaseObject.quoteName(".'cart_product` cp
                            LEFT JOIN `'.dataBaseObject.quoteName(".'category_product` catp ON cp.id_product = catp.id_product
                            WHERE cp.`id_cart` = '.(int)$context->cart->id.'
                            AND cp.`id_product` IN ('.implode(',', array_map('intval', $eligible_products_list)).')
                        ...
                        case 'manufacturers':
                            $cart_manufacturers = Db::getInstance()->executeS('
                                ...
                                FROM `'.dataBaseObject.quoteName(".'cart_product` cp
                            LEFT JOIN `'.dataBaseObject.quoteName(".'product` p ON cp.id_product = p.id_product
                            WHERE cp.`id_cart` = '.(int)$context->cart->id.'
                            AND cp.`id_product` IN ('.implode(',', array_map('intval', $eligible_products_list)).')');
                            $count_matching_products = 0;
                            $matching_products_list = array();
    ...
    protected function getCartRuleCombinations($offset = null, $limit = null, $search = '')
    {
        ...
        $array['selected'] = Db::getInstance()->executeS('
            ...
            FROM '.dataBaseObject.quoteName(".'cart_rule cr
        LEFT JOIN '.dataBaseObject.quoteName(".'cart_rule_lang crl ON (cr.id_cart_rule = crl.id_cart_rule AND crl.id_lang = '.(int)Context::getContext()->language->id.')
        ...
        AND (
                ...
                OR EXISTS (
                        SELECT 1
                        FROM '.dataBaseObject.quoteName(".'cart_rule_combination
        WHERE cr.id_cart_rule = '.dataBaseObject.quoteName(".'cart_rule_combination.id_cart_rule_1 AND '.(int)$this->id.' = id_cart_rule_2
        )
        OR EXISTS (
            SELECT 1
        FROM '.dataBaseObject.quoteName(".'cart_rule_combination
        WHERE cr.id_cart_rule = '.dataBaseObject.quoteName(".'cart_rule_combination.id_cart_rule_2 AND '.(int)$this->id.' = id_cart_rule_1
        )
        ...
        $array['unselected'] = Db::getInstance()->executeS('
            SELECT cr.*, crl.*, 1 as selected
            FROM '.dataBaseObject.quoteName(".'cart_rule cr
        INNER JOIN '.dataBaseObject.quoteName(".'cart_rule_lang crl ON (cr.id_cart_rule = crl.id_cart_rule AND crl.id_lang = '.(int)Context::getContext()->language->id.')
        LEFT JOIN '.dataBaseObject.quoteName(".'cart_rule_combination crc1 ON (cr.id_cart_rule = crc1.id_cart_rule_1 AND crc1.id_cart_rule_2 = '.(int)$this->id.')
        LEFT JOIN '.dataBaseObject.quoteName(".'cart_rule_combination crc2 ON (cr.id_cart_rule = crc2.id_cart_rule_2 AND crc2.id_cart_rule_1 = '.(int)$this->id.')
    ...
    public function getAssociatedRestrictions($type, $active_only, $i18n, $offset = null, $limit = null, $search_cart_rule_name = '')
    {
        ...
        FROM `'.dataBaseObject.quoteName(".$type.'` t
        '.($i18n ? 'LEFT JOIN `'.dataBaseObject.quoteName(".$type.'_lang` tl ON (t.id_'.$type.' = tl.id_'.$type.' AND tl.id_lang = '.(int)Context::getContext()->language->id.')' : '').'
        WHERE 1
        ...
        if ($type == 'cart_rule') {
            ...
            FROM `'.dataBaseObject.quoteName(".$type.'` t
            '.($i18n ? 'LEFT JOIN `'.dataBaseObject.quoteName(".$type.'_lang` tl ON (t.id_'.$type.' = tl.id_'.$type.' AND tl.id_lang = '.(int)Context::getContext()->language->id.')' : '').'
            LEFT JOIN (SELECT id_'.$type.' FROM `'.dataBaseObject.quoteName(".'cart_rule_'.$type.'` WHERE id_cart_rule = '.(int)$this->id.') crt ON t.id_'.($type == 'carrier' ? 'reference' : $type).' = crt.id_'.$type.'
    ...
    public static function autoAddToCart(Context $context = null)
    {
        ...
        SELECT SQL_NO_CACHE cr.*
            FROM '.dataBaseObject.quoteName(".'cart_rule cr
        LEFT JOIN '.dataBaseObject.quoteName(".'cart_rule_shop crs ON cr.id_cart_rule = crs.id_cart_rule
        '.(!$context->customer->id && Group::isFeatureActive() ? ' LEFT JOIN '.dataBaseObject.quoteName(".'cart_rule_group crg ON cr.id_cart_rule = crg.id_cart_rule' : '').'
        LEFT JOIN '.dataBaseObject.quoteName(".'cart_rule_carrier crca ON cr.id_cart_rule = crca.id_cart_rule
        '.($context->cart->id_carrier ? 'LEFT JOIN '.dataBaseObject.quoteName(".'carrier c ON (c.id_reference = crca.id_carrier AND c.deleted = 0)' : '').'
        LEFT JOIN '.dataBaseObject.quoteName(".'cart_rule_country crco ON cr.id_cart_rule = crco.id_cart_rule
        WHERE cr.active = 1
        AND cr.code = ""
        AND cr.quantity > 0
        ...
        '.($context->customer->id ? 'OR EXISTS (
            SELECT 1
        FROM `'.dataBaseObject.quoteName(".'customer_group` cg
        INNER JOIN `'.dataBaseObject.quoteName(".'cart_rule_group` crg ON cg.id_group = crg.id_group
        ...
        OR EXISTS (
            SELECT 1
        FROM `'.dataBaseObject.quoteName(".'cart_product`
        WHERE `'.dataBaseObject.quoteName(".'cart_product`.`id_product` = cr.`reduction_product` AND `id_cart` = '.(int)$context->cart->id.'
        )
        )
        AND NOT EXISTS (SELECT 1 FROM '.dataBaseObject.quoteName(".'cart_cart_rule WHERE cr.id_cart_rule = '.dataBaseObject.quoteName(".'cart_cart_rule.id_cart_rule
