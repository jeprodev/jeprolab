...
public class JeproLabCategoryModel extends JeproLabModel{
    ...
    public List<Integer> laboratory_ids = new ArrayList<>();
    ...
    public static boolean multi_lang = true;
    public static boolean multi_lang_lab = true;
    ...
    public boolean isMultiLab(){
        return JeproLabLaboratoryModel.isTableAssociated("category") || JeproLabCategoryModel.multi_lang_lab;
    }

    public boolean isLangMultiLab(){
        return JeproLabCategoryModel.multi_lang && JeproLabCategoryModel.multi_lang_lab;
    }
    ...
    public boolean existsInLaboratories(int labId){
        ...
        //dataBaseObject.setQuery(query);
        return dataBaseObject.loadValue(query, "category_id") > 0;
    }
            ...
            if(result != null) {
                try {
                    ...
                    while (result.next()) {
                        ...
                        category.name.put("lang_" + langId, result.getString("name"));
                        category.description.put("lang_" + langId, result.getString("description"));
                        category.link_rewrite.put("lang_" + langId, result.getString("link_rewrite"));
                        category.meta_keywords.put("lang_" + langId, result.getString("meta_keywords"));
                        category.meta_title.put("lang_" + langId, result.getString("meta_title"));
                        category.meta_description.put("lang_" + langId, result.getString("meta_description"));
                        categories.add(category);
                    }
                } catch (SQLException ignored) {
                    JeproLabUncaughtExceptionHandler.logExceptionMessage(Level.ERROR, ignored);
            ...
            if(resultSet != null) {
                try{
                    ...
                    while(resultSet.next()){
                        ...
                        category.description.put("lang_" + langId, resultSet.getString("description"));
    ...
    public boolean hasMultiLabEntries() {
        ...
        return dataBaseObject.loadValue(query, "entries") > 0;
    }
    ...
    public void saveCategory(){
        ...
        if(!context.controller.has_errors){
            ...
            if(JeproLabLaboratoryModel.isTableAssociated("category")){
                ...
                if(this.laboratory_ids.size() > 0){ labListIds = this.laboratory_ids; }
            }
    ...
    public boolean delete(){
        ...
        for(JeproLabCategoryModel category : allCategories) {
            /** @var Category $cat */
            //todo category.liteDelete();
            if (!this.hasMultiLabEntries()) {
                //Todo category.deleteImage();
                category.cleanGroups();
                category.cleanAssociatedAnalyzes();
                // Delete associated restrictions on cart rules
                categories = new ArrayList<>();
                categories.add(category.category_id);
                JeproLabCartModel.JeproLabCartRuleModel.cleanAnalyzeRuleIntegrity("categories", categories);
                JeproLabCategoryModel.cleanPositions(category.parent_id);
                /* Delete Categories in GroupReduction */
                if (JeproLabGroupModel.JeproLabGroupReductionModel.getGroupsReductionByCategoryId(category.category_id).size() > 0) {
    ...
    public static int getLastPosition(int categoryParentId, int labId){
        ...
        //dataBaseObject.setQuery(query);
        return (1 + (int)dataBaseObject.loadValue(query, "max_pos"));
    }
    ...
    public int getDuplicatePosition(){
        ...
        //dataBaseObject.setQuery(query);
        return (int)dataBaseObject.loadValue(query, "category_id");
    }
    ...
    public List<JeproLabCategoryModel> getAllChildren(int langId) {
        ...
        if(resultSet != null){
            try{
                JeproLabCategoryModel category;
                while(resultSet.next()){
                    category = new JeproLabCategoryModel(resultSet.getInt("category_id"));
                    categories.add(category);
                }
            }catch (SQLException ignored){
                JeproLabUncaughtExceptionHandler.logExceptionMessage(Level.ERROR, ignored);
