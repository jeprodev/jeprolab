...
public class JeproLabAnalyzeController extends JeproLabController {
    ...
    public TableColumn<JeproLabAnalyzeRecord, CheckBox> jeproLabAnalyzeCheckBoxColumn;
    public TableColumn<JeproLabAnalyzeRecord, String> jeproLabAnalyzeReferenceColumn, jeproLabAnalyzeNameColumn;
    public TableColumn<JeproLabAnalyzeRecord, Button> jeproLabAnalyzeStatusColumn;
    //public TableColumn;
    ...
    public void initialize(URL location, ResourceBundle resource){
        ...
        Callback<TableColumn<JeproLabAnalyzeRecord, CheckBox>, TableCell<JeproLabAnalyzeRecord, CheckBox>> checkBoxCellFactory = param -> new JeproLabAnalyzeCheckBoxCell();
        ...
        Callback<TableColumn<JeproLabAnalyzeRecord, Button>, TableCell<JeproLabAnalyzeRecord, Button>> statusCellFactory = param -> new JeproLabAnalyzeStatusCell();
    ...
    public class JeproLabAnalyzeRecord{
    ...
    private class JeproLabAnalyzeCheckBoxCell extends TableCell<JeproLabAnalyzeRecord, CheckBox>{
        private CheckBox analyzeCheckBox;

        public JeproLabAnalyzeCheckBoxCell(){
            analyzeCheckBox = new CheckBox();
        }

        @Override
        public void commitEdit(CheckBox t){
            super.commitEdit(t);
        }
    ...
    private class JeproLabAnalyzeStatusCell extends TableCell<JeproLabAnalyzeRecord, Button>{
        private Button analyzeStatus;

        public JeproLabAnalyzeStatusCell(){
    ...
    private class JeproLabAnalyzeActionCell extends TableCell<JeproLabAnalyzeRecord, HBox> {
        private HBox commandContainer;
        private Button editAnalyze, deleteAnalyze, viewAnalyze;

        public JeproLabAnalyzeActionCell(){
            commandContainer = new HBox(1);
            ...
            editAnalyze.setTooltip(tooltip);
            editAnalyze.setOnMouseClicked(event -> {
                final ObservableList<JeproLabAnalyzeRecord> items = getTableView().getItems();
                int analyzeId = items.get(getIndex()).getAnalyzeId();
                JeproLab.request.setRequest("analyze_id=" + analyzeId + "&reference=" + items.get(getIndex()).getAnalyzeReference());
            ...
            viewAnalyze = new Button("", new ImageView(new Image(JeproLab.class.getResourceAsStream("resources/images/view.png"))));
            viewAnalyze.setPrefSize(btnSize, btnSize);
            viewAnalyze.setMaxSize(btnSize, btnSize);
            viewAnalyze.setMinSize(btnSize, btnSize);
            viewAnalyze.getStyleClass().add("icon-btn");
            viewAnalyze.setTooltip(tooltip);
            viewAnalyze.setOnMouseClicked(new EventHandler<MouseEvent>() {
                @Override
                public void handle(MouseEvent event) {
                    System.out.println("editit function");
                }
            });
            viewAnalyze.setOnMouseEntered(new EventHandler<MouseEvent>() {
                @Override
                public void handle(MouseEvent event) {
                    tooltip.setText(bundle.getString("JEPROLAB_VIEW_CATEGORY_MESSAGE"));
                }
            });
            commandContainer.getChildren().addAll(editAnalyze, viewAnalyze, deleteAnalyze);
        }
