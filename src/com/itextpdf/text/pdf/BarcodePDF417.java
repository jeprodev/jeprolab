/*
 * $Id: e6b2f0e27b7feb219a84dc2fde3671fdcf11ef62 $
 *
...
public class BarcodePDF417 {
    ...
    private void textCompaction(byte[] input, int start, int length) {
        ...
        if (size + cwPtr > MAX_DATA_CODEWORDS) {
            throw new IndexOutOfBoundsException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); // todo ComposedMessage("the.text.is.too.big"));
        }
        length = ptr;
        ptr = 0;
        while (ptr < length) {
    ...
    private void macroCodes() {
        if (macroSegmentId < 0) {
            throw new IllegalStateException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); // todo ComposedMessage("macrosegmentid.must.be.gt.eq.0"));
        }
        if (macroSegmentId >= macroSegmentCount) {
            throw new IllegalStateException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); // todo ComposedMessage("macrosegmentid.must.be.lt.macrosemgentcount"));
        }
        if (macroSegmentCount < 1) {
            throw new IllegalStateException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); // todo ComposedMessage("macrosemgentcount.must.be.gt.0"));
        }
    ...
    public void paintCode() {
        ...
        if ((options & PDF417_USE_RAW_CODEWORDS) != 0) {
            if (lenCodewords > MAX_DATA_CODEWORDS || lenCodewords < 1 || lenCodewords != codewords[0]) {
                throw new IllegalArgumentException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); // todo ComposedMessage("invalid.codeword.size"));
            }
        }
        else {
            if (text == null)
                throw new NullPointerException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); // todo ComposedMessage("text.cannot.be.null"));
