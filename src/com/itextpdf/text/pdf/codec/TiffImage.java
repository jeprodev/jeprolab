...
public class TiffImage {
    ...
    public static Image getTiffImage(RandomAccessFileOrArray s, boolean recoverFromImageError, int page, boolean direct) {
        if (page < 1)
            throw new IllegalArgumentException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); //todoComposedMessage("the.page.number.must.be.gt.eq.1"));
        try {
            TIFFDirectory dir = new TIFFDirectory(s, page - 1);
            if (dir.isTagPresent(TIFFConstants.TIFFTAG_TILEWIDTH))
                throw new IllegalArgumentException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); //todo ComposedMessage("tiles.are.not.supported"));
        ...
        try {
            ...
            switch (compression) {
                ...
                case TIFFConstants.COMPRESSION_JPEG:
                    break;
                default:
                    throw new IllegalArgumentException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); //todo ComposedMessage("the.compression.1.is.not.supported", compression));
            }
            ...
            switch (photometric) {
                ...
                default:
                    if (compression != TIFFConstants.COMPRESSION_OJPEG && compression != TIFFConstants.COMPRESSION_JPEG)
                        throw new IllegalArgumentException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); //todo ComposedMessage("the.photometric.1.is.not.supported", photometric));
            }
            float rotation = 0;
            if (dir.isTagPresent(TIFFConstants.TIFFTAG_ORIENTATION)) {
                int rot = (int)dir.getFieldAsLong(TIFFConstants.TIFFTAG_ORIENTATION);
            ...
            if (dir.isTagPresent(TIFFConstants.TIFFTAG_PLANARCONFIG)
                ...
                throw new IllegalArgumentException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); //todo ComposedMessage("planar.images.are.not.supported"));
            int extraSamples = 0;
            ...
            switch (bitsPerSample) {
                ...
                case 8:
                    break;
                default:
                    throw new IllegalArgumentException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); //todo ComposedMessage("bits.per.sample.1.is.not.supported", bitsPerSample));
            }
            Image img = null;
            ...
            if (compression == TIFFConstants.COMPRESSION_LZW || compression == TIFFConstants.COMPRESSION_DEFLATE || compression == TIFFConstants.COMPRESSION_ADOBE_DEFLATE) {
                ...
                if (predictorField != null) {
                    ...
                    if (predictor != 1 && predictor != 2) {
                        throw new RuntimeException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); //todo ComposedMessage("illegal.value.for.predictor.in.tiff.file"));
                    }
                    if (predictor == 2 && bitsPerSample != 8) {
                        throw new RuntimeException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); //todo ComposedMessage("1.bit.samples.are.not.supported.for.horizontal.differencing.predictor", bitsPerSample));
                    }
                }
            }
            ...
            if (compression == TIFFConstants.COMPRESSION_OJPEG) {
                ...
                if ((!dir.isTagPresent(TIFFConstants.TIFFTAG_JPEGIFOFFSET))) {
                    throw new IOException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); //todo ComposedMessage("missing.tag.s.for.ojpeg.compression"));
                }
                ...
                if (size.length > 1)
                    throw new IOException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); // todo ComposedMessage("compression.jpeg.is.only.supported.with.a.single.strip.this.image.has.1.strips", size.length));
    ...
    static Image ProcessExtraSamples(DeflaterOutputStream zip, DeflaterOutputStream mzip, byte[] outBuf, int samplePerPixel, int bitsPerSample, int width, int height) throws IOException {
        if (bitsPerSample == 8) {
            ...
            throw new IllegalArgumentException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); //todo ComposedMessage("extra.samples.are.not.supported"));
        return null;
    }
