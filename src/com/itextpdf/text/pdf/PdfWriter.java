...
//	the PdfDocument instance
    ...
    public void setInitialLeading(final float leading) throws DocumentException {
    	if (open)
    		throw new DocumentException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_YOU_CANT_SET_THE_INITIAL_LEADING_IF_THE_DOCUMENT_IS_ALREADY_OPENED_MESSAGE"));
    ...
    public PdfContentByte getDirectContent() {
        if (!open)
            throw new RuntimeException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_THE_DOCUMENT_IS_NOT_OPEN_MESSAGE"));
 ...
 */
    ...
    public PdfIndirectReference getPageReference(int page) {
        --page;
        if (page < 0)
            throw new IndexOutOfBoundsException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_THE_PAGE_NUMBER_MUST_BE_GREATER_OR_EQUALS_TO_ONE_MESSAGE"));
    ...
    PdfIndirectReference add(final PdfPage page, final PdfContents contents) throws PdfException {
        if (!open) {
            throw new PdfException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_THE_DOCUMENT_IS_NOT_OPEN_MESSAGE"));
        }
...
//	Open and Close methods + method that create the PDF
	...
	/**
	 * @see PdfVersion#addDeveloperExtension(com.itextpdf.text.pdf.PdfDeveloperExtension)
	 * @since	2.1.6
	 */
...
// [C6] Actions (open and additional)
    ...
    /** @see PdfDocumentActions#setOpenAction(String) */
...
//  [C8] AcroForm
    ...
    /** @see PdfAnnotations#addAnnotation(com.itextpdf.text.pdf.PdfAnnotation) */
    ...
    /** @see PdfAnnotations#addCalculationOrder(com.itextpdf.text.pdf.PdfFormField) */
...
//  [C10] PDFX Conformance
    ...
    public void setPDFXConformance(final int pdfx) {
        ...
        if (((PdfXConformance)pdfIsoConformance).getPDFXConformance() == pdfx)
            return;
        if (pdf.isOpen())
            throw new PdfXConformanceException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_PDFX_CONFORMANCE_CAN_ONLY_BE_SET_BEFORE_OPENING_THE_DOCUMENT_MESSAGE"));
        if (crypto != null)
            throw new PdfXConformanceException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_PDFX_CONFORMING_DOCUMENT_CANNOT_BE_ENCRYPTED_MESSAGE"));
        if (pdfx != PDFXNONE)
...
//  [F1] PdfEncryptionSettings interface
    ...
    /** @see PdfEncryptionSettings#setEncryption(byte[], byte[], int, int) */
    public void setEncryption(final byte userPassword[], final byte ownerPassword[], final int permissions, final int encryptionType) throws DocumentException {
        if (pdf.isOpen())
            throw new DocumentException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_ENCRYPTION_CAN_ONLY_BE_ADDED_BEFORE_OPENING_THE_DOCUMENT_MESSAGE"));
        crypto = new PdfEncryption();
        crypto.setCryptoMode(encryptionType, 0);
        crypto.setupAllKeys(userPassword, ownerPassword, permissions);
    }

    /** @see PdfEncryptionSettings#setEncryption(Certificate[], int[], int) */
...
//  [F2] compression
    ...
    public void setFullCompression() throws DocumentException {
    	if (open)
    		throw new DocumentException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_YOU_CANT_SET_THE_FULL_COMPRESSION_IF_THE_DOCUMENT_IS_ALREADY_OPEN_MESSAGE"));
...
//  [F12] tagged PDF
    ...
    public void setTagged(int taggingMode) {
        if (open)
            throw new IllegalArgumentException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_TAGGING_MUST_BE_SET_BEFORE_OPENING_THE_DOCUMENT_MESSAGE"));
        tagged = true;
    ...
    public void checkElementRole(IAccessibleElement element, IAccessibleElement parent) {
        if (parent != null && (parent.getRole() == null || PdfName.ARTIFACT.equals(parent.getRole())))
            ...
            if (element.isInline() && element.getRole() == null && (parent == null || !parent.isInline()))
                throw new IllegalArgumentException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_INLINE_ELEMENTS_WITH_ROLE_NULL_ARE_NOT_ALLOWED_MESSAGE"));
        }
    }
...
//  [U3] page actions (open and close)
    ...
    /** @see PdfPageActions#setTransition(com.itextpdf.text.pdf.PdfTransition) */
