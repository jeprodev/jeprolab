    ...
    public static Rectangle getRectangle(String name)  {
        ...
        if (pos == -1) {
            try {
                ...
                throw new RuntimeException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); // todo ComposedMessage("can.t.find.page.size.1", name));
            }
        }
        else {
        	try {
        		...
        		throw new RuntimeException(MessageLocalization.getErrorBundle().getString("ITEXTPDF_MESSAGE")); // todo ComposedMessage("1.is.not.a.valid.page.size.format.2", name, e.getMessage()));
        	}
        }
    }
}
